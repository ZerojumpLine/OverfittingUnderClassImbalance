...
class Cnn3d(object):
    ...
    def setup_ops_n_feeds_to_train(self, log, total_cost, updates_of_params_wrt_total_cost) :
        ...
        y_gt_mix0 = self._output_gt_tensor_feeds['train']['y_gt0']
        y_gt_mix1 = self._output_gt_tensor_feeds['train']['y_gt1']
        mixup_lambda = self._output_gt_tensor_feeds['train']['mixup_lambda']
        ...
        self._ops_main['train']['list_rp_rn_tp_tn'] = self.finalTargetLayer.getRpRnTpTnForTrain0OrVal1(y_gt_mix0, 0)
        ...
        for subpath_i in range(self.numSubsPaths) : # if there are subsampled paths...
            ...
            self._feeds_main['train']['d_sub_' + str(subpath_i)] = self._disturb_x['train']['x_sub_' + str(subpath_i)]
        self._feeds_main['train']['d'] = self._disturb_x['train']['x']
        self._feeds_main['train']['y_gt0'] = y_gt_mix0
        self._feeds_main['train']['y_gt1'] = y_gt_mix1
        self._feeds_main['train']['mixup_lambda'] = mixup_lambda
    ...
    def _setupInputXTensors(self):
        ...
        self._disturb_x['train']['x'] = tf.placeholder(dtype="float32", shape=[None, None, None, None, None], name="disturb_x_train")
        ...
        for subpath_i in range(self.numSubsPaths) : # if there are subsampled paths...
            ...
            self._disturb_x['train']['x_sub_' + str(subpath_i)] = tf.placeholder(dtype="float32", shape=[None, None, None, None, None], name="disturb_x_sub_" + str(subpath_i) + "_train")
    ...
    def make_cnn_model( self,
        ...
        inputToPathwayTrain = self._inp_x['train']['x'] + self._disturb_x['train']['x']
        ...
        for subpath_i in range(self.numSubsPaths) :
            ...
            inputToPathwayTrain = self._inp_x['train']['x_sub_' + str(subpath_i)] + self._disturb_x['train']['x_sub_' + str(subpath_i)]
        ...
        (self._output_gt_tensor_feeds['train']['y_gt0'],
         self._output_gt_tensor_feeds['train']['y_gt1'],
         self._output_gt_tensor_feeds['train']['mixup_lambda'],
         self._output_gt_tensor_feeds['val']['y_gt']) = self.finalTargetLayer.get_output_gt_tensor_feed()

        # Adversarial training ###################################################################
        # I should output the distance direction in which adversarial is the minimum..
    ...
    def advfunc(self):
        ...
        # it should be output in the first iteration
        self.d0 = get_normalized_vector(d1)
