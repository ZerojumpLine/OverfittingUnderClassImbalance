...
def load_subj_and_get_samples(log,
    ...
    ## do not flip in the image level.
    ...
    for cat_i in range(samplingTypeInstance.getNumberOfCategoriesToSample()):
        ## do augmentation based on the category
        ## when the center is tumor, implement the augmentation.
        ...
        for image_part_i in range(len(coords_of_samples[0][0])):
            ...
            if Augmflag:
                ...
                reflectFlags = []
                for reflectImageWithHalfProb_dimi in range(0, len(reflectImageWithHalfProbDuringTraining)):
                    reflectFlags.append(reflectImageWithHalfProbDuringTraining[reflectImageWithHalfProb_dimi] * random.randint(0, 1))
                for pathway_i in range(len(channelsForThisImagePartPerPathway)):
                    channelsForThisImagePartPerPathway[pathway_i][0] = reflectImageArrayIfNeeded(reflectFlags, channelsForThisImagePartPerPathway[pathway_i][0])
                gtLabelsForTheCentralClassifiedPartOfThisImagePart = reflectImageArrayIfNeeded(reflectFlags, gtLabelsForTheCentralClassifiedPartOfThisImagePart)
            for pathway_i in range(cnn3d.getNumPathwaysThatRequireInput()):
    ...
    del allChannelsOfPatientInNpArray;
    del gtLabelsImage;
    del roiMask
    del arrayWithWeightMapsWhereToSampleForEachCategory;
    del allSubsampledChannelsOfPatientInNpArray;
    del tupleOfPaddingPerAxesLeftRight
...
def load_imgs_of_subject(log,
    ...
    if providedGtLabelsBool:  # For training (exact target labels) or validation on samples labels.
        ...
        [imageGtLabels, tupleOfPaddingPerAxesLeftRight] = padCnnInputs(imageGtLabels, cnnReceptiveField, dimsOfPrimeSegmentRcz) if padInputImagesBool else [imageGtLabels,
    ...
    if train_val_or_test != "test" and providedWeightMapsToSampleForEachCategory == True:  # in testing these weightedMaps are never provided, they are for training/validation only.
        ...
        for cat_i in range(numberOfSamplingCategories):
            ...
            [weightedMapForThisCatData, tupleOfPaddingPerAxesLeftRight] = padCnnInputs(weightedMapForThisCatData, cnnReceptiveField, dimsOfPrimeSegmentRcz) if padInputImagesBool else [
...
def getImagePartFromSubsampledImageForTraining(dimsOfPrimarySegment,
    ...
    rToCentralVoxelOfAnAveragedArea = subSamplingFactor[0] // 2 if subSamplingFactor[0] % 2 == 1 else (
                subSamplingFactor[0] // 2 - 1)  # one closer to the beginning of dim. Same happens when I get parts of image.
    ...
    rhighNonIncl = int(rlow + subSamplingFactor[0] * recFieldCnn[0] + (math.ceil((numberOfCentralVoxelsClassifiedForEachImagePart_rDim * 1.0) / subSamplingFactor[0]) - 1) * subSamplingFactor[
        0])  # excluding index in segment
...
def extractDataOfASegmentFromImagesUsingSampledSliceCoords(
    ...
    # Intensity augmentation
    if train_or_val == "train" and doIntAugm_shiftMuStd_multiMuStd[0] == True and Augmflag:
    ...
    for pathway_i in range(len(cnn3d.pathways)):  # Except Normal 1st, cause that was done already.
        ...
        slicesCoordsOfSegmForPrimaryPathway = [[leftBoundaryRcz[0], rightBoundaryRcz[0] - 1], [leftBoundaryRcz[1], rightBoundaryRcz[1] - 1],
                                               [leftBoundaryRcz[2], rightBoundaryRcz[2] - 1]]  # rightmost  are placeholders here.
    ...
    for segment_i in range(numberOfSegmentsToExtract):
        ...
        for pathway_i in range(len(cnn3d.pathways)):  # Except Normal 1st, cause that was done already.
            ...
            slicesCoordsOfSegmForPrimaryPathway = [[rLowBoundary, rFarBoundary - 1], [cLowBoundary, cFarBoundary - 1],
                                                   [zLowBoundary, zFarBoundary - 1]]  # the right hand values are placeholders in this case.
