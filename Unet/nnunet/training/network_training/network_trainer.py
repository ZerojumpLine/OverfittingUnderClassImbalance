...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.tr_gen = self.tr_RAW = self.val_gen = None
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            for b in range(self.num_batches_per_epoch):
                l = self.run_iterationRAW(self.tr_gen, self.tr_RAW, self.raw, self.probBGGetAugment, True)
    ...
    def run_iterationRAW(self, data_generator, data_generatorRAW, raw, probBGGetAugment, do_backprop=True, run_online_evaluation=False):
        data_dict = next(data_generator)
        data = data_dict['data']
        target = data_dict['target']
        data_dictRAW = next(data_generatorRAW)
        ...
        if probBGGetAugment > random.uniform(0,1):
            ...
            # do not use augmentation for BG samples
            # the 0 is the BG + FG, I do not want augmentation on this
            data[0, :, :, :, :] = dataRAW[0, :, :, :, :]
            target[0, :, :, :, :] = targetRAW[0, :, :, :, :]
            # the 1 is only FG, I should remain as it is.
        ...
        ########################### adversarial trianing ###########################
        ## the first step, train using the normal samples

        self.optimizer.zero_grad()
        output = self.network(data)
        l = self.mloss(output, target)
        ...
        if not isinstance(dataasy, torch.Tensor):
            dataasy = torch.from_numpy(dataasy).float()
        if not isinstance(targetasy, torch.Tensor):
            targetasy = torch.from_numpy(targetasy).float()

        dataasy = dataasy.cuda(non_blocking=True)
        targetasy = targetasy.cuda(non_blocking=True)

        ## the second step, find the direction
        if self.eps > 0:
            r_vadv = self.generate_virtual_adversarial_perturbation(dataasy, targetasy)
            r_vadv = r_vadv.detach()
            ## the third step, train using the argumented results
            self.optimizer.zero_grad()
            output = self.network(dataasy + r_vadv)

            l_adv = self.mloss(output, targetasy)
            if do_backprop:
                if not self.fp16 or amp is None:
                    l_adv.backward()
                else:
        ...
        if self.margin > 0:
            ...
            self.optimizer.zero_grad()
            # normal training
            output = self.network(datamixup)
            lmix = self.mixloss(output, targetasy, lam)
            if do_backprop:
                if not self.fp16 or amp is None:
                    lmix.backward()
                else:
                    with amp.scale_loss(lmix, self.optimizer) as scaled_loss:
                        scaled_loss.backward()
                self.optimizer.step()
            del datamix
            del datamixup
    ...
    def generate_virtual_adversarial_perturbation(self, data, target):
        d = torch.randn(data.size())
        d = d.cuda(non_blocking=True)
        d = self.xi * self.get_normalized_vector(d)
        # print(torch.min(d))
        # print(torch.min(data))
        # print(torch.mean(torch.abs(d)))
        # print(torch.mean(torch.abs(data)))
        d.requires_grad = True
        logit_m = self.network(data + d)
        dist = self.advCEloss(logit_m, target)
        grad = torch.autograd.grad(dist, d)[0] # return a tuple?
        d = grad.detach()

        return self.eps * self.get_normalized_vector(d)
