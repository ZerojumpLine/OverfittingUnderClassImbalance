...
from nnunet.training.loss_functions.dice_loss import DC_and_CE_loss, advCE_loss, mCE_loss, mixCE_loss
...
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False, asy=0, xi=1e-5, eps=10, raw=True, initlr = 3e-4,
                 marginm = 0, gama = 0, alpha = 0.2, margin = 0, weights = 1, inner = 1, probBGGetAugment=1.0):
        """
        :param deterministic:
        ...
        self.eps = eps
        self.raw = raw
        self.probBGGetAugment = probBGGetAugment
        ...
        self.advCEloss = advCE_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False, 'square': False}, {'asy': asy})
        self.mloss = mCE_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False, 'square': False},
                              {'asy': asy, 'gama':gama, 'margin': marginm, "weights": weights})
        self.mixloss = mixCE_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False, 'square': False},
                                  {'asy': asy, 'margin': margin, 'inner': inner, 'gama':gama, 'marginm': marginm, "weights": weights})
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        self.folder_with_preprocessed_data = join(self.dataset_directory, self.plans['data_identifier'] +
                                                  "_stage%d" % self.stage)
        if training:
            self.dl_tr, self.dl_trRAW, self.dl_val = self.get_basic_generators()
            if self.unpack_data:
            ...
            self.tr_gen, self.tr_RAW, self.val_gen = get_default_augmentation(self.dl_tr, self.dl_trRAW, self.dl_val,
    ...
    def get_basic_generators(self):
        ...
        if self.threeD:
            ...
            dl_trRAW = DataLoader3D(self.dataset_tr, self.patch_size, self.patch_size, self.batch_size,
                                    False, oversample_foreground_percent=self.oversample_foreground_percent,
                                    pad_mode="constant", pad_sides=self.pad_all_sides)
